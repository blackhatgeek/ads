...
begin
	...
	for i:=0 to 9 do begin
		for j:=0 to 9 do if souc_bez[j,i].p then write('[',souc_bez[j,i].c,'] ') else write(souc_bez[j,i].c,' ');
		writeln('');
	end;
	writeln('');
	...
	souc_s[9,9].c:=0;
	for i:=0 to 9 do begin
		for j:=0 to 9 do if souc_s[j,i].p then write('[',souc_s[j,i].c,'] ') else write(souc_s[j,i].c,' ');
		writeln('');
	end;
	{ODCITANI}
	{naplnim pole pro stav bez prenosu}
